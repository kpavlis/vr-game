...
public class Interactor : MonoBehaviour
{
    ...
    private IInteractable lastInteractable; //saved last object for calculations between checks/frames
    ...
    void Update()
    {
        ...
        if (!interacting)
        { //if you aren't currently interacting

            if (readyInteract && Input.GetKeyDown(interactKey))
            { //...but you are ready to interact AND pressing interact button
        ...
        { //if interacting
            ...
            { //is pressing any cancel button (movement buttons for example)
